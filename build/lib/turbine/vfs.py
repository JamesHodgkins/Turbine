...
class VirtualFileSystem:
    ...
    def get_snapshot(self, key: str) -> list[str] | None:
        """Return the current (post-staged-edits) lines for a file, or None if unknown."""
        lines = self._snapshots.get(key)
...
def _replay_hunks(baseline: list[str], hunks: list[Hunk]) -> list[str]:
    ...
    for hunk in sorted(hunks, key=lambda h: h.orig_start):
        # Convert 1-based orig_start to 0-based index, clamped to 0 so that
        # "insert at beginning" hunks (orig_start == 0 from diffing empty
        # files) are handled correctly rather than resolving to -1 and
        # accidentally excluding the last line via Python negative indexing.
